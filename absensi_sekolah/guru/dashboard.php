...
$mapel_user = isset($_SESSION['mapel']) ? $_SESSION['mapel'] : '-';
...
if (isset($_POST['absen'])) {
    ...
    if (mysqli_num_rows($cek) > 0) {
        ...
        // PERBAIKAN DI SINI: Kolom 'mapel' ditambahkan ke dalam query INSERT
        mysqli_query($conn, "INSERT INTO presensi (nama, role, mapel, waktu_absen) VALUES ('$nama_user', 'guru', '$mapel_user', NOW())");
...
<body style="display: block; background: #f4f7f6;">
    ...
    <div style="background: #2c3e50; color: white; padding: 15px 40px; display: flex; justify-content: space-between; align-items: center;">
        <h2 style="margin:0; font-size: 1.1rem;">E-Absensi Guru</h2>
    ...
    <div class="container-absen">
        <div class="card-absen">
            <h1 style="color: #2d3436; margin-bottom: 5px;">Selamat Pagi, <?php echo $nama_user; ?>!</h1>
            <p style="color: #0984e3; font-weight: bold; margin-top: 0;">Mata Pelajaran: <?php echo $mapel_user; ?></p>
            ...
            <table>
                ...
                <tbody>
                    ...
                    if(mysqli_num_rows($res) == 0) {
                        echo "<tr><td colspan='4' style='text-align:center; color:#b2bec3;'>Belum ada data absensi masuk hari ini.</td></tr>";
                    }
                    while($row = mysqli_fetch_assoc($res)) {
                        $mapel_tampil = !empty($row['mapel']) ? $row['mapel'] : '-';
                        echo "<tr>
                                <td>".date('H:i', strtotime($row['waktu_absen']))." WIB</td>
                                <td style='font-weight:600;'>{$row['nama']}</td>
                                <td>{$mapel_tampil}</td>
                                <td><span class='badge badge-guru'>HADIR</span></td>
                              </tr>";
                    }
                    ?>
                </tbody>
